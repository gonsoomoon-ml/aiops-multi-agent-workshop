#!/usr/bin/env bash
# teardown.sh — Phase 6a Change Runtime + 의존 자원 reverse 순서 삭제.
# Phase 0/2/3/4 자원 + 다른 Phase 6a 자원 (monitor_a2a, incident_a2a, supervisor) 미터치.
# infra/phase6a/ Lambda + Target 은 별 teardown (`infra/phase6a/teardown.sh`) — 본 스크립트 범위 밖.
# reference: phase4 incident teardown.sh 동일 골격.
set -euo pipefail

REGION="${AWS_REGION:-us-west-2}"
DEMO_USER="${DEMO_USER:?DEMO_USER 미설정}"
AGENT_NAME="aiops_demo_${DEMO_USER}_change"
...
echo -e "${YELLOW}=== Phase 6a Change teardown — ${AGENT_NAME} ===${NC}"
...
# ── .env cleanup ────────────────────────────────────────────────
if [ -f "${SCRIPT_DIR}/.env" ]; then
    sed -i.bak '/^RUNTIME_ARN=/d; /^RUNTIME_ID=/d; /^RUNTIME_NAME=/d; /^OAUTH_PROVIDER_NAME=/d; /^CHANGE_RUNTIME_ARN=/d; /^# Phase 6a Runtime/d' "${SCRIPT_DIR}/.env"
    rm -f "${SCRIPT_DIR}/.env.bak"
    echo -e "  ${GREEN}✓ ${SCRIPT_DIR}/.env 의 Phase 6a Change entry cleanup${NC}"
fi

# ── Phase 4 자원 보존 검증 (negative check) ─────────────────────
echo -e "${YELLOW}[verify] Phase 4 자원 보존 검증${NC}"
INCIDENT_ID=$(aws bedrock-agentcore-control list-agent-runtimes --region "$REGION" \
    --query "agentRuntimes[?agentRuntimeName=='aiops_demo_${DEMO_USER}_incident'].agentRuntimeId" \
    --output text 2>/dev/null || echo "")
if [ -n "$INCIDENT_ID" ] && [ "$INCIDENT_ID" != "None" ]; then
    echo -e "  ${GREEN}✓ Phase 4 Incident Runtime 보존 (${INCIDENT_ID})${NC}"
else
    echo -e "  ${RED}❌ Phase 4 Incident Runtime 미발견${NC}"
fi
...
if [ -n "$MONITOR_ID" ] && [ "$MONITOR_ID" != "None" ]; then
    ...
    echo -e "  ${RED}❌ Phase 3 Monitor Runtime 미발견${NC}"
fi

echo -e "${GREEN}=== ✅ Phase 6a Change teardown 완료 ===${NC}"
echo -e "${YELLOW}NOTE: deployments-storage Lambda + Target 은 'bash infra/phase6a/teardown.sh' 로 별도 정리${NC}"
