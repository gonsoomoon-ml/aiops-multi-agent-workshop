#!/usr/bin/env bash
# teardown.sh — Phase 5 Supervisor Runtime + 의존 자원 reverse 순서 삭제.
# Phase 0/2/3/4 자원 + 다른 Phase 5 자원 (monitor_a2a, incident_a2a) 미터치.
set -euo pipefail
...
echo -e "${YELLOW}=== Phase 5 Supervisor teardown — ${AGENT_NAME} ===${NC}"
...
echo -e "${GREEN}=== ✅ Phase 5 Supervisor teardown 완료 ===${NC}"
