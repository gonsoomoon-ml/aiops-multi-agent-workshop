...
def copy_shared_into_build_context() -> None:
    """[1/5] monitor_a2a/shared 를 build context 로 복사. self-contained."""
    print(f"{YELLOW}[1/5] shared/ 를 빌드 컨텍스트로 복사 중...{NC}")

    shared_src = PROJECT_ROOT / "agents" / "monitor_a2a" / "shared"
    if not shared_src.exists():
        print(f"{RED}❌ monitor_a2a/shared 미발견: {shared_src}{NC}")
        sys.exit(1)
