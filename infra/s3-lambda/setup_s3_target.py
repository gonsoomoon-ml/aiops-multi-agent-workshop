"""S3 backend — Gateway Target 's3-storage' 추가 (boto3, idempotent).

GitHub backend 의 setup_github_target.py 와 형제. cognito-gateway 의 Gateway 에
새 Target 1건만 추가 — Gateway / Cognito / 다른 Target 미터치.

deploy.sh 가 CFN outputs 를 환경변수로 주입한 뒤 호출:
    GATEWAY_ID                 (cognito-gateway .env carry-over)
    S3_STORAGE_LAMBDA_ARN      (s3-lambda CFN output)

reference:
    - infra/github-lambda/setup_github_target.py (형제 패턴)
    - infra/cognito-gateway/setup_gateway.py:create_target (보존 패턴)
"""
import os
import sys
...
S3_TOOL_SCHEMA = [
    {
        ...
        "description": (
            ...
            "and fetches data/runbooks/<alarm-class>.md from S3 "
            ...
            "Note: 응답 shape 는 github-storage 와 동일 — Incident Agent 는 backend 무관."
        ),
        "inputSchema": {
            "type": "object",
            "required": ["alarm_name"],
...
def create_or_update_target(gw, gateway_id: str, lambda_arn: str):
    ...
    fail. update 분기로 재배포 시에도 lambdaArn + S3_TOOL_SCHEMA 가 항상 최신.
    """
    print(f"\n=== S3 backend — GatewayTarget '{TARGET_NAME}' 추가/갱신 ===")

    target_config = {
        "mcp": {
            "lambda": {
                "lambdaArn": lambda_arn,
                "toolSchema": {"inlinePayload": S3_TOOL_SCHEMA},
            }
        }
    }
...
def main():
    required = ["GATEWAY_ID", "S3_STORAGE_LAMBDA_ARN"]
    missing = [k for k in required if not os.environ.get(k)]
    if missing:
        print(f"환경변수 누락: {missing}", file=sys.stderr)
        print("deploy.sh 가 CFN outputs + cognito-gateway .env 를 export 한 뒤 호출하세요.", file=sys.stderr)
        sys.exit(1)
    ...
    create_or_update_target(
        ...
        lambda_arn=os.environ["S3_STORAGE_LAMBDA_ARN"],
    )

    print(f"\n✅ S3 backend — s3-storage Target 등록 완료 (Gateway 무변경, Target 1건 추가)")
