#!/usr/bin/env bash
# infra/phase4/deploy.sh — GitHub Lambda CFN + Gateway Target (boto3) 배포.
#
...
#
# reference:
#   - infra/phase2/deploy.sh (cfn package + setup_gateway 패턴)
#   - docs/design/phase4.md §6-2 (smoke test 절차)
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$PROJECT_ROOT/infra/phase4"
...
GITHUB_STORAGE_LAMBDA_ARN="$GITHUB_STORAGE_LAMBDA_ARN" \
    uv run python "$PROJECT_ROOT/infra/phase4/setup_github_target.py"
