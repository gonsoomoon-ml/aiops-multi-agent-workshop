#!/usr/bin/env bash
# infra/phase2/deploy.sh — Cognito + 2 Lambda + IAM (CFN) + Gateway + 2 Target (boto3)
set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$PROJECT_ROOT/infra/phase2"
...
# ── 1. mock_data 복사 (Lambda zip 에 포함) ───────
COPY_DIR="$PROJECT_ROOT/infra/phase2/lambda/history_mock/mock_data"
log "mock_data/phase1 → Lambda 디렉토리 복사"
rm -rf "$COPY_DIR"
mkdir -p "$COPY_DIR"
touch "$COPY_DIR/__init__.py"
cp -r "$PROJECT_ROOT/mock_data/phase1" "$COPY_DIR/phase1"

# ── 2. cfn package (Lambda Code 디렉토리 zip + S3 업로드) ─
log "cfn package — Lambda Code 디렉토리 zip + S3 업로드"
aws cloudformation package \
    --template-file "$PROJECT_ROOT/infra/phase2/cognito.yaml" \
    --s3-bucket "$DEPLOY_BUCKET" \
    --s3-prefix "phase2-cognito" \
    --region "$REGION" \
    --output-template-file "$PROJECT_ROOT/infra/phase2/cognito.packaged.yaml" >/dev/null

# ── 3. CFN deploy (Cognito + Lambda + IAM 통합) ──
log "CFN deploy: $STACK"
aws cloudformation deploy \
    --region "$REGION" \
    --template-file "$PROJECT_ROOT/infra/phase2/cognito.packaged.yaml" \
    --stack-name "$STACK" \
...
DEMO_USER="$DEMO_USER" AWS_REGION="$REGION" \
    uv run python "$PROJECT_ROOT/infra/phase2/setup_gateway.py" \
    | tee "$TMP_OUT"
...
log "  검증: P2-A1~A5 (docs/design/phase2.md Section 8)"
