#!/usr/bin/env bash
# infra/phase2/teardown.sh — Gateway/Target → CFN stack → DEPLOY_BUCKET → .env 정리
set -euo pipefail
...
DEMO_USER="$DEMO_USER" AWS_REGION="$REGION" \
    uv run python "$PROJECT_ROOT/infra/phase2/cleanup_gateway.py" || warn "cleanup_gateway.py 실패 (무시 후 진행)"
...
# ── 4. 복사된 mock_data 정리 (deploy 부산물) ────
rm -rf "$PROJECT_ROOT/infra/phase2/lambda/history_mock/mock_data"
rm -f "$PROJECT_ROOT/infra/phase2/cognito.packaged.yaml"
